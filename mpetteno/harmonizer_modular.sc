...
// ~crossfbModeControlBuses = Array.fill(~voiceNumber, {arg i; Bus.control(s, 1)});
...
SynthDef.new(\pitchShifter, { arg channelIndex, gain = 0.0;
	...
	var input, delayedSignal, pitchShiftInput, isPitchShiftFeedbackMode, pitchRatio, pitchShiftedSignal;
...
SynthDef.new(\feedbackDelayLine, { arg channelIndex, delayTime = 0.014, feedbackAmount = 0.0;
	...
	var input, feedbackNode, delayedSignal, feedbackSignal, pitchShiftedSignal, isPitchShiftFeedbackMode;
...
voiceChannels.do({ arg voiceChannel, index;
	...
	crossfbModeCheckbox.action_({
		~~crossfbModeControlBuses[index].set(\isCrossFeedbackMode, crossfbModeCheckbox.value);
	});*/
